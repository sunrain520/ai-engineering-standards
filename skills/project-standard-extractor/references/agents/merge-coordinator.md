...
把通过 Quality Gate 的结果**以 append-only 方式写入规范目录**，并把跨 batch 的**维度激活态信息聚合到端 overview 文档的 §9 未激活维度地图**，保持知识库的可追溯性和一致性。分层知识库模型（extracted→draft→active）下，Merge Coordinator 是 extracted→draft 的最后一道门——写入即固化，不可隐性覆盖。

> 上游：`review-and-quality-gate`（quality_gate_decisions + review_report + activation_report 透传）。下游：规范目录（负责人手工发布）+ overview 未激活维度地图（AE7 强制）。

## 输入

```yaml
inputs:
  quality_gate_decisions: []  # 每条规则一个决策（含 dimension_id / dimension_state / final_gate_decision）
  review_report:              # 包含 debate_records / persona_findings / shallow_overrides / pending_overrides / candidate_dim_summary
  activation_report:          # temp/{run_id}-activation-report.json（schema=activation-report.v1，透传自 review）
...
existing_index:
  docs:
    - doc_id: ""
      ...
      sub_domain: ""
      indexable: false
  rule_locators:     # [{source_doc, section_title, level, status, dimension_id, dimension_state}]
    - source_doc: ""
      section_title: ""
      level: ""
      status: ""
      dimension_id: ""        # 关联到 activation-report.dimensions[]
      dimension_state: ""     # baseline / activated / pending-confirmation / shallow / candidate
  ...
  overview_doc:             # 当前 domain 的 overview 文档定位
    path: ""                # 如 engineering-standards/04-backend/standard-backend-overview.md
...
本阶段对 `quality_gate_decisions[].dimension_state` 与 `activation_report.dimensions[]` 做双重消费：
...
| `candidate` | **不写入端规范**；汇总到 overview-{domain}.md §9「未激活维度地图」 | overview-{domain}.md §9 |

**铁律**：本阶段不重新判定 state；activation-report 是唯一权威源。任何与 activation-report / quality_gate_decisions[].dimension_state 不一致的写入视为 schema violation。
...
4. 多项目模式下 `overview-{domain}.md §9 未激活维度地图` 仍按 unified_candidate 维度写入；`partial_activated` 不进 §9（它在 §3 差异列已显性出现）。
...
merge_summary:
  ...
  activation_report_ref: ""    # 透传：temp/{run_id}-activation-report.json（review-summary.md 引用）
  activation_report_path: ""   # Step 4.6 落盘后的正式路径：{output_dir}/{domain}/evidence/dimension-activation-report.json
  written_files: []            # 写入或追加的文件列表
  new_rule_locators: []        # 新增 [{source_doc, section_title, level, dimension_id, dimension_state}]
...
### Step 1.5 — Activation Report 加载与门禁

1. 读取 `activation_report` JSON，校验 `schema == "activation-report.v1"`；不匹配抛 `ACTIVATION_REPORT_SCHEMA_INVALID`。
2. 校验 `run_id` 与当前 run 一致；不匹配抛 `ACTIVATION_REPORT_RUN_MISMATCH`。
3. 读取 `review_report` 中的 `shallow_overrides[]`、`pending_overrides[]`、`candidate_dim_summary{}`。
4. 把 `activation_report.dimensions[]` 按 state 分组并写入 `merge_summary.activation_summary`（5 个计数）。
5. 校验**激活态-决策一致性**（强校验）：
   ...
   - 任一 `quality_gate_decision.dimension_state == "pending-confirmation"` 但 `final_gate_decision == "merge-to-active"` → 抛 `PENDING_FORCED_TO_ACTIVE`，停止写入。
...
### Step 2 — 决策路由（含维度激活态分桶）

**决策优先级**：先按 `dimension_state` 决定写入目录大类（baseline / activated / shallow / pending / candidate），再按 `final_gate_decision` 与 `target_state` 决定文件级路由。
...
| `activated` | `merge-to-active` / `keep-draft` | `standard-{sub_domain}.md` + `ai-rules.md` + `review-checklist.md` + `evidence/*` | 主路径 |
...
| `candidate` | （P8 已 BLOCK，不应出现 final_gate_decision；理论上不进入本步骤） | `overview-{domain}.md §9`（仅地图条目，不写规则） | 由 Step 4.5 单独聚合 |

**target_state 二级路由**（仅对 baseline/activated/shallow 适用）：

| target_state 条件 | 写入位置 | 说明 |
| --- | --- | --- |
| target_state=draft, recommended_action=keep-draft | `standard-{sub_domain}.md` + `ai-rules.md` + `review-checklist.md` + `evidence/*` | 主路径 |
...
### Step 4.5 — Overview §9 未激活维度地图聚合（AE7 强制段，跨 batch 持久化）

**触发**：每次 batch 合并完成后**必须**执行；即使本 batch 无 candidate 维度，仍需校验 overview §9 段标题存在（`preserved_when_empty: true`）。

**执行步骤**：

1. **定位 overview 文档**：
   - 路径：`{target_domain_dir}/standard-{domain}-overview.md`（如 `engineering-standards/04-backend/standard-backend-overview.md`）
...
5. **记录到 merge_summary**：
   ...
   overview_section_9:
     written: true
     overview_path: "engineering-standards/04-backend/standard-backend-overview.md"
...
### Step 4.6 — Activation Report 持久化（R54 强制落盘 / U17）

每次 run 必须把 `temp/{run_id}-activation-report.json` 拷贝并补字段后落盘到正式 evidence 目录,作为下次增量 run 的基线 + 跨项目合并源 + Coverage Reviewer 复核的产物附属物。
...
6. 把落盘路径回写 `merge_summary.activation_report_path`(替代原 `activation_report_ref` 指向 temp/)。
...
- [ ] `pending-confirmation.md` 包含所有 `target_state: pending-confirmation` 的规则与所有 `dimension_state: pending-confirmation` 的维度，含 `promotion_criteria`
...
- [ ] 所有 `dimension_state == "shallow"` 的规则在 standard 中带 low-coverage 标注，且在 ai-rules.md 中带 ⚠️ warning
- [ ] overview-{domain}.md §9 已建段（即使 candidate=0 也保留标题）
...
- [ ] `merge_summary.activation_report_ref` 指向有效路径
...
- [ ] `existing_index.rule_locators[]` 每条都包含 `dimension_id` 与 `dimension_state` 字段（不允许遗漏）
...
| `failed` | activation_report 与 quality_gate_decisions[].dimension_state 不一致 | `STATE_INCONSISTENCY` | step 10b 回滚 |
...
| activation_report.json 缺失或 schema 不匹配 | `ACTIVATION_REPORT_SCHEMA_INVALID`；停止写入，要求重跑 dimension-activator |
...
| pending-confirmation 维度被强制为 merge-to-active | `PENDING_FORCED_TO_ACTIVE`；停止；要求 review 重跑 |
...
4. 记录每次运行的 `merge_summary`（包含 AI 使用警告、负责人确认项、activation_summary 三态计数）。
5. 首次大批量合并前建议执行 `dry_run: true`。
6. **每次合并完成都必须更新 overview-{domain}.md §9 未激活维度地图**——AE7 强制段，即使 candidate=0 也保留章节标题。
7. **rule_locators[] 每条都必须带 `dimension_id` 与 `dimension_state`**——这是 review→merge 的契约字段，不是可选元数据。
8. **激活态-决策一致性强校验**（Step 1.5）：candidate 不进 standard、pending 不变 active、shallow 必带 low-coverage——任一违反立即抛错停止写入。
9. **§9 历史 candidate 条目永久保留**（append-only / soft-archive）；即使本 run 维度变 activated，也只在「最近更新」列追加 `→ activated since {run_id}`，不删除原条目。
10. activation_report.json 透传到 `merge_summary.activation_report_ref`，用于 review-summary.md 引用。
11. **每次 run 必须执行 Step 4.6 把 activation-report 持久化到 `evidence/dimension-activation-report.json`**（R54 / U17 强制；落盘路径回写 `merge_summary.activation_report_path`）；增量模式生成 `evolution{}` 段；跨项目模式同时落盘 per-project + unified map + 主报告三件套。
...
7. **不得本地重新计算 dimension_state**——activation_report 与 quality_gate_decisions[].dimension_state 是唯一权威源，merge 阶段不得擅自调整。
8. **不得把 candidate 维度规则写入 standard/ai-rules.md/review-checklist.md**——candidate 维度只能进 overview §9 地图。
9. **不得把 pending-confirmation 维度强制升为 active 或 merge-to-active**——只能写 pending-confirmation.md。
10. **不得跳过 Step 4.5 §9 写入**——即使本 batch 全部激活，也必须校验 §9 段标题存在。
11. **不得删除 overview §9 中的历史 candidate 条目**（append-only 铁律），只能 soft-archive 标注 `→ activated since {run_id}`。
12. **不得在 §9 表格中混入 activated/baseline/shallow/pending 维度**——§9 仅展示 candidate 状态。
