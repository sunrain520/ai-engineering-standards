...
| 项 | guide 模式默认 | catalog 模式默认 | 说明 |
| --- | --- | --- | --- |
| profile 阶段代表性目录深度 | 3 层 | 3 层 | 不变 |
| 每个 batch 候选文件数 | 30 | 15 | guide 需覆盖 sub_domain 多个角色/层级 |
| 每个 batch evidence 条目数 | 25 | 8 | guide 需要足够 evidence 写完整指南 |
| 每次正式萃取 batch 数 | 1 | 1 | 不变 |

`guide` 模式放宽预算是因为：综合 Developer Guide 需要覆盖 sub_domain 所有主要层级，读 8 个文件只能覆盖 1-2 个角色，无法产出有实质价值的完整指南。
