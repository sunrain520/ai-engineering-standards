...
- `.gitignore` 已含 `tools/maintainer/project-standard-extractor/.local-backups/`
- `engineering-standards/01-app-client/` 含完整 phase 2 产物(11 baseline 维度全 draft)
- `tools/maintainer/project-standard-extractor/.local-backups/01-app-client/` 当前已有 5 份历史备份(无 pinned)
...
- backup_dir = `tools/maintainer/project-standard-extractor/.local-backups/01-app-client/<UTC-ts>/` 创建,含 manifest.json + sha256 + 完整 domain 内容
...
- 释放 `tools/maintainer/project-standard-extractor/.local-backups/01-app-client/.lock`
...
- `tools/maintainer/project-standard-extractor/.local-backups/01-app-client/<UTC-ts>/failure.log` 写入 magic header `force-rebuild-failure.v1` + `failure_reason: QUALITY_GATE_BLOCKED`
...
- `head -1 tools/maintainer/project-standard-extractor/.local-backups/01-app-client/<ts>/failure.log` == `force-rebuild-failure.v1`
...
- Terminal A 已对 `01-app-client` 取得 lock(`mkdir tools/maintainer/project-standard-extractor/.local-backups/01-app-client/.lock` 成功)并正在 phase 2 执行
...
**额外断言**: Terminal A 失败 / 中断时 lock 不被自动清(避免 stuck);host developer 手动 `rmdir tools/maintainer/project-standard-extractor/.local-backups/01-app-client/.lock` 才能释放。
...
- `tools/maintainer/project-standard-extractor/.local-backups/01-app-client/<UTC-ts>/payload/` 仅作为备份 payload,不参与本失败路径回滚
...
- `tools/maintainer/project-standard-extractor/.local-backups/01-app-client/` 已有 12 份备份,其中第 6 份(`<UTC-ts-6>`) `manifest.json.pin = false`
...
- `tools/maintainer/project-standard-extractor/.local-backups/04-backend/<UTC-ts-2>/manifest.rollback` 段填充
...
head -1 tools/maintainer/project-standard-extractor/.local-backups/<domain>/<ts>/failure.log
...
# 5. lock 清理
test ! -d tools/maintainer/project-standard-extractor/.local-backups/<domain>/.lock && echo "lock-released" || echo "LOCK-LEAK"
